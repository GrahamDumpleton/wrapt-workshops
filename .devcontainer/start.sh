...
# public. Remote access is allowed because requests arrive with the
# forwarded github.dev host name rather than localhost.
#
...
# everything JupyterLab starts would inherit them: kernels, checks and
# anything a learner runs in a cell. No workshop here needs GitHub
# access. env -u drops the token variables (GITHUB_CODESPACE_TOKEN only
# in case Codespaces sets it; unsetting a variable that is not set does
# nothing), and the GIT_CONFIG_* variables give git an empty
